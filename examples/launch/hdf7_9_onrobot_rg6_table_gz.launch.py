...
def generate_launch_description():
    ...
    if existing_gz_path:
        gz_resource_path = f"{existing_gz_path}:{resource_path}:{simulation_pkg}"
    else:
        gz_resource_path = f"{resource_path}:{simulation_pkg}"
...
def launch_setup(context, *args, **kwargs):
    ...
    xacro_file = os.path.join(
        description_pkg, "examples", "hdf7_9_onrobot_rg6.urdf.xacro"
    )
    controllers_file = os.path.join(
        simulation_pkg, "examples", "config", "hdf7_9_onrobot_rg6_controllers.yaml"
    )
    rviz_config_file = os.path.join(
        description_pkg, "rviz", "display_robot.rviz"
    )
    world_file = os.path.join(
        simulation_pkg, "examples", "world", "table_world.sdf"
    )
