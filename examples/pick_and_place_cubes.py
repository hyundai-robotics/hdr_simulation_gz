...
from moveit_msgs.msg import (
    RobotState, Constraints, MoveItErrorCodes,
    CollisionObject, PlanningScene, JointConstraint,
)
...
class PickAndPlace(Node):
    ...
    def add_collision_objects(self):
        ...
        t1.id = "table_1"
        t1.operation = CollisionObject.ADD
        t1.primitives.append(SolidPrimitive(type=SolidPrimitive.BOX, dimensions=[1.0, 1.5, 0.05]))
        p1 = Pose()
        p1.position.z = -0.025
        p1.orientation.w = 1.0
        t1.primitive_poses.append(p1)
        scene.world.collision_objects.append(t1)

        # Table 2 - cube table
        t2 = CollisionObject()
        t2.header.frame_id = self.base_frame
        t2.id = "table_2"
        t2.operation = CollisionObject.ADD
        t2.primitives.append(SolidPrimitive(type=SolidPrimitive.BOX, dimensions=[1.0, 1.5, 0.05]))
        p2 = Pose()
        p2.position.x = 1.0
        p2.position.z = -0.025
        p2.orientation.w = 1.0
        t2.primitive_poses.append(p2)
        scene.world.collision_objects.append(t2)
    ...
    def run(self):
        ...
        sequence = [
            ('pick_1_approach',  (0.736, 0.189, 0.282),  orient, 'close'),
            ('pick_1_lift',      (0.730, 0.107, 0.580),  orient, None),
            ('place_1',          (0.660, -0.207, 0.273), orient, 'open'),
            ('place_1_retreat',  (0.592, -0.221, 0.516), orient, None),
            ('pick_2_approach',  (0.726, -0.014, 0.288), orient, 'close'),
            ('pick_2_lift_1',    (0.668, -0.026, 0.497), orient, None),
            ('pick_2_lift_2',    (0.514, -0.126, 0.393), orient, None),
            ('place_2',          (0.609, -0.181, 0.315), orient, 'open'),
            ('place_2_retreat',  (0.592, -0.221, 0.516), orient, None),
        ]

        for name, pos, orient, action in sequence:
